...
@Controller
@RequestMapping("admin/manage-file/image")
public class ManageImageController {
    ...
    public String getAllImage(Model model) {
        ...
        return "quan-ly-file/file-image";
    }
    ...
    public String saveImage(@Valid @ModelAttribute("resourceDto") ResourceDto resourceDto,
        ...
        if(resourceDto.getId() != null && resourceDto.getMultipartFile().getOriginalFilename().equals("")){
            // không thay đổi
            return "redirect:/admin/manage-file/image";
        }else{
            Resource resource = resourceService.saveFile(resourceDto, TypeResource.IMAGE.getValue());
            if(resource != null){
