...
function initData(){
    ...
    if(questionsApi != null){
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-5">
                <input type="text" class="form-control" id="audio-input" name="audio-input" placeholder="Link audio"  value = "${questionsApi.audio.linkAudio}">
            </div>
            <div class="col-lg-3 text-center">
                <input type="file" class="btn mb-1 btn-rounded btn-success" id="audio-file" style="width: inherit;"><span class="btn-icon-left"></span>
                <span class="btn-icon-left file-name" id="span-audio-file">${questionsApi.audio.mutipartFileAudio ? questionsApi.audio.mutipartFileAudio.split('/').pop() : 'No file chosen'}</span>
            </div>
        </div>
      ...
      // Thêm sự kiện change
           ...
           const fileAudioNameSpan = document.getElementById(`span-audio-file`);
           audioInput.addEventListener('change', function() {
            // Lấy danh sách các tệp đã chọn
            const selectedFiles = this.files;
            
            // Kiểm tra nếu có ít nhất một tệp được chọn
            if (selectedFiles.length > 0) {
                // Lấy tên của tệp đầu tiên trong danh sách và cập nhật vào thẻ span
                const fileName = selectedFiles[0].name;
                fileAudioNameSpan.textContent = fileName;
            } else {
                // Nếu không có tệp nào được chọn, cập nhật thông báo tương ứng vào thẻ span
                fileAudioNameSpan.textContent = 'No file chosen';
            }
            });
}
...
function createQuestionObject() {
    ...
    var questionObject = {
        ...
        audio: {
            nameAudio: nameAudio,
            linkAudio: linkAudio,
            mutipartFileAudio: formDataAudio
        },
        questions: questions
