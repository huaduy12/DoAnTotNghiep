...
import com.example.client_toeic.repository.RelatedDocumentRepository;
...
public class RelatedDocumentServiceImpl implements RelatedDocumentService {
    ...
    @Override
    public List<RelatedDocument> findAllByType(String type) {
        return relatedDocumentRepository.findAllByType(type);
    }

    @Override
    public List<RelatedDocumentDto> findAllByOther(Integer id) {
        List<RelatedDocument> relatedDocuments = relatedDocumentRepository.findAllByNotId(id);
        List<RelatedDocumentDto> relatedDocumentDtos = modelMapper.map(relatedDocuments,new TypeToken<List<RelatedDocumentDto>>(){}.getType());
        List<Integer> relatedDocumentIds = relatedDocuments.stream().map(RelatedDocument::getId).collect(Collectors.toList());
        List<Resource> resources = resourceRepository.findByRelatedDocumentIdIn(relatedDocumentIds);
        Map<Integer, Resource> mapResources = resources.stream()
                .collect(Collectors.toMap(Resource::getRelatedDocumentId, Function.identity()));
        for (RelatedDocumentDto relatedDocument: relatedDocumentDtos) {
            Resource resource = mapResources.get(relatedDocument.getId());
            relatedDocument.setFileName(resource.getFileName());
            relatedDocument.setLinkImage(resource.getImage());
        }
        return relatedDocumentDtos;
    }
