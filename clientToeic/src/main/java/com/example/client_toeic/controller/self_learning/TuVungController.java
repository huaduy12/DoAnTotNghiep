...
@Controller
@RequestMapping("/client/self-learning/vocabulary")
public class TuVungController {
    ...
    public String pageNews(Model model, @RequestParam(value = "page",defaultValue = "1",required = false) Integer page){
        ...
        Page<SelfLearningDto> selfLearningDtos = selfLearningService.getAll(TypeExercise.TUHOC_TUVUNG.getValue(),page);
        model.addAttribute("selfLearningDtos",selfLearningDtos);
        model.addAttribute("page",page);
        return "/tu-hoc/self-learning";
    }
    ...
    public String pageNews(@PathVariable("id") Integer id, Model model){
        ...
        model.addAttribute("selfLearningDtoOthers",selfLearningService.getLearningOther(TypeExercise.TUHOC_TUVUNG.getValue(), id));
        ...
        return "/tu-hoc/detail-self-learning-vocabulary";
    }
}
